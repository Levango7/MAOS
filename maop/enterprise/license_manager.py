...
class LicenseManager:
    ...
    private_key_path
        ...
        a fresh keypair is generated in-memory (useful for tests).
    public_key_path
    ...
    def _load_private_key(path: Path | None) -> Any:
        """Load the Ed25519 private key for signing. Generate if None."""
    ...
    def _load_public_key(
        ...
        Resolution order:
          ...
          3. ``loaded_private_key`` is an in-memory key → use its public
             key directly (covers the ``private_key_path=None`` case where
             we generated a fresh keypair in :meth:`_load_private_key`).
          4. Fall back to the bundled ``maop.enterprise.keys/public_key.pem``
             (verification-only mode — signing will produce unverifiable
             keys, but this is useful for read-only inspection of licenses
             signed by the production key).
        """
        from cryptography.hazmat.primitives import serialization
        ...
        # In-memory private key (generated by _load_private_key when path is None).
        if loaded_private_key is not None:
            return loaded_private_key.public_key(), "in-memory-paired"
        # Last resort: bundled public key (verification-only).
        ...
        from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
        return Ed25519PrivateKey.generate().public_key(), "in-memory-fallback"
    ...
    def delete_license(self, license_id: str) -> bool:
        """Delete a license and its audit logs. Returns True if deleted."""
        # Verify existence first.
        self.get_license(license_id)
        # Record the deletion audit entry BEFORE removing the license row,
        # because license_audit_logs has a FOREIGN KEY → licenses(license_id).
        # We then delete both the audit logs and the license row together.
        self._log_audit(
            license_id=license_id,
            action="deleted",
        ...
        if self._pg:
            deleted: bool = self._pg.delete_license(license_id)
            return deleted
        with self._sqlite_connect() as conn:
            conn.execute("DELETE FROM license_audit_logs WHERE license_id=?", (license_id,))
            conn.execute("DELETE FROM licenses WHERE license_id=?", (license_id,))
        logger.info("[license_mgr] Deleted license_id=%s", license_id)
        return True
