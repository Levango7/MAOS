...
## 整改跟踪（2026-08-29 更新，v5.2.1）

本审计报告中提出的部分建议已在 v5.2.0 + v5.2.1 落地，状态更新如下（v5.2.1
新增项以「★ 5.2.1」标注）：
...
| SAML 重放防护（v5.2.0 实现实为死代码） | ⚠️ 实现但每次新建 handler 致 InResponseTo/重放表恒空 | ✅ **★ 5.2.1 激活**（P0）：`SSOManager` 改为进程内复用同一 `SAMLHandler` 实例，重放防护真正生效 |
| OIDC userinfo 失败降级 `oidc:unknown`（多用户身份合并） | ⚠️ 认证缺陷 | ✅ **★ 5.2.1 fail-closed**（P0，Breaking）：userinfo 获取失败或缺失 `sub` 拒绝登录 |
